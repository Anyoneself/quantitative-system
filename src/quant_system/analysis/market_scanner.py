...
from quant_system.analysis.scanner import ScanResult
from quant_system.analysis.service import analyze_stock
from quant_system.data.errors import DataRefreshError
from quant_system.data.source import iter_a_stock_symbols
...
class MarketScanner:
    ...
    def start(self, algorithm: str, top_n: int, interval_seconds: int) -> None:
        with self._lock:
            self._run_id += 1
            run_id = self._run_id
            ...
            self._thread = threading.Thread(
                ...
                args=(run_id, self._stop_event, algorithm, top_n, interval_seconds),
                daemon=True,
            )
            self._thread.start()
    ...
    def _status_message_locked(self) -> str:
        ...
        if self._status == "waiting":
            return f"本轮全量完成，下一轮筛选 {self._next_run_at}"
    ...
    def _run_loop(
        ...
        while not stop_event.is_set() and self._is_current_run(run_id):
            if not self._run_once(run_id, stop_event, algorithm, top_n):
                break
            ...
            with self._lock:
                if self._run_id != run_id:
                    break
                self._next_run_at = _format_time(next_run_at)
                self._status = "waiting"
                self._mark_changed_locked()
            while time.time() < next_run_at and not stop_event.is_set() and self._is_current_run(run_id):
                time.sleep(1)
