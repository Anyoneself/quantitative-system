...
from quant_system.analysis.market_scanner import MARKET_SCANNER
from quant_system.analysis.scanner import parse_symbols, scan_top_stocks
from quant_system.analysis.service import build_advice
from quant_system.data.errors import DataRefreshError
from quant_system.data.source import fetch_bars
from quant_system.output.formatter import advice_to_dict
...
class QuantWebHandler(SimpleHTTPRequestHandler):
    ...
    def _handle_market_scan_start(self) -> None:
        try:
            ...
            algorithm = str(payload.get("algorithm", "knn"))
            top_n = _parse_int(payload.get("top"), 10)
            interval_seconds = _parse_int(payload.get("interval_seconds"), 600)
            if algorithm not in ALGORITHMS:
                self._send_json({"ok": False, "error": "机器学习算法不支持。"}, status=400)
                return
            MARKET_SCANNER.start(algorithm, top_n, interval_seconds)
            self._send_json({"ok": True, "snapshot": asdict(MARKET_SCANNER.snapshot())})
        except json.JSONDecodeError:
            self._send_json({"ok": False, "error": "请求不是有效 JSON。"}, status=400)
...
def _beginner_payload(advice) -> dict:
    ...
    return {
        ...
        "plain_language": [
            "机器学习评分不是收益承诺，更准确地说，是历史相似量价样本里的上涨占比。",
